...
function Icon({ name, size=18 }:{ name:"arrow"|"refresh"|"search"|"close"|"share"|"check"|"github"; size?:number }) {
  const paths:Record<string,React.ReactNode> = {
    arrow:<><path d="M5 12h14"/><path d="m13 6 6 6-6 6"/></>, refresh:<><path d="M20 11a8 8 0 1 0-2.3 5.7"/><path d="M20 4v7h-7"/></>, search:<><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></>, close:<><path d="m6 6 12 12M18 6 6 18"/></>, share:<><path d="M12 16V3m0 0L7 8m5-5 5 5"/><path d="M5 13v7h14v-7"/></>, check:<path d="m5 12 4 4 10-10"/>, github:<path d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.604-3.369-1.341-3.369-1.341-.454-1.155-1.11-1.462-1.11-1.462-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0 1 12 6.836a9.6 9.6 0 0 1 2.504.337c1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48A10.02 10.02 0 0 0 22 12c0-5.523-4.477-10-10-10z"/>,
  };
...
export default function Home() {
  ...
  return <main className="site-shell">
    ...
    <header className="nav wrap"><button className="brand" onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}><strong>昨天本来可以</strong><span>by.QingHan</span></button><nav><button className="location-button" onClick={()=>setCityOpen(true)}><span className="location-dot"/>{location.name}<Icon name="arrow" size={15}/></button><a className="text-button" href="https://github.com/catQing/Yesterday" target="_blank" rel="noopener noreferrer" aria-label="GitHub"><Icon name="github" size={18}/>GitHub</a><button className="text-button" onClick={()=>setAboutOpen(true)}>关于</button></nav></header>
